...
class Flux_ScoopIt extends Flux_Site {
	...
	public function __construct($tokenStore, $myUrl, $consumerKey, $consumerSecret, $httpBackend = null, $scoopitServerUrl = null, $idBase=false, $bTrace=false)
    {
	...
	public function login(){
		...
		if($accessToken == null) {
			...
			if($accessToken==null){
				...
				$urlVerif = $this->scitAuthorizeUrl."?oauth_token=$requestToken&oauth_callback=".urlencode($this->myUrl);
				Header("Location: ".$urlVerif);
				exit;
			}
		}
	}
	...
	public function createAPost($title, $url, $content, $imageUrl, $topicId, $resolveUrl=false) {
		$data = "action=create&title=".urlencode($title)."&url=".urlencode($url)
			."&content=".urlencode($content)."&imageUrl=".urlencode($imageUrl)."&topicId=".$topicId
	...
	public function prepareAPost($url) {
		$data = "action=prepare&url=".urlencode($url);
